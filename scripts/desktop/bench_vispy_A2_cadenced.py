...
def main() -> None:
    p = argparse.ArgumentParser(description="VisPy A2 cadenced benchmark (lateness-aware).")
    ...
    def on_draw(event) -> None:
        ...
        latency_ms.append(float(t_present - issued_ms[pid]))
        lateness_ms.append(float(t_present - scheduled_ms[pid]))
    ...
    for i, (rx0, rx1) in enumerate(ranges):
        sched = t_start + i * interval
        scheduled_ms.append(sched)
        ...
        summary.update(
            {
                ...
                "lateness_max_ms": float(np.max(a)),
            }
        )
